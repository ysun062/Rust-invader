...
use crate::{
    components::{Enemy, SpriteSize, FromEnemy, Velocity, Laser, Movable},
    EnemyCount, GameTextures, WinSize, ENEMY_MAX, ENEMY_SIZE, SPRITE_SCALE, ENEMY_LASER_SIZE, BASE_SPEED, TIME_STEP,
};
use bevy::{prelude::*, core::FixedTimestep, ecs::schedule::ShouldRun, transform};
...
fn enemy_spawn_system(
    ...
    if enemy_count.0 < ENEMY_MAX {
        ...
        let formation: Formation = formation_maker.make(&win_size);
...
fn enemy_movement_system(time: Res<Time>, mut query: Query<(&mut Transform, &mut Formation), With<Enemy>>) {
    ...
    for (mut transform, mut formation) in query.iter_mut() {
        ...
        // 1 for couter clockwise, -1 for clockwise
